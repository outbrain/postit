...
PostIt.prototype.on = function (event, listener) {
	...
	var wrapListener = function (evt) {
		// RETURN without parsing `evt.data` - a catch all `listener` (native behavior).
		if ('*' === event) {
		...
		// RETURN without invoking `listener`, if an event/id/origin (excluding an asterisk origin) does not match.
		if (event !== data.__event ||
			that.id !== data.__id ||
			'*' !== data.__origin && -1 === window.location.href.indexOf(data.__origin)
		) {
			return;
		}
