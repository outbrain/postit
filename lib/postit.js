...
PostIt.prototype.off = (function () {
	var removeListener = function (listeners, index) {
		window.removeEventListener('message', listeners[index], false);
	...
	return function (event, listener) {
		...
		for (var i = 0; i < listeners.length; ++i) {
			...
			if (!listener) {
				...
				--i;
			} else if (listener.__token === listeners[i].__token) {
