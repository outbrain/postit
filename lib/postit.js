...
PostIt.prototype.on = function (event, listener) {
	...
	var wrapListener = function (evt) {
		...
		// RETURN EARLY if a message is sent from a non-PostIt `event`.
		if (!/__event/.test(evt.data)) {
			return;
		}
...
PostIt.prototype.off = function (event, listener) {
	...
	// Remove an explicit `event` `listener` for a given `event`.
	if (listener) {
		for (var i = 0; i < listeners.length; ++i) {
			if (listener.__token === listeners[i].__token) {
	...
	while ((_listener = listeners.shift())) {
...
PostIt.prototype.emit = function (event, target, message, origin) {
	...
	switch (Object.prototype.toString.call(message)) {
		case '[object Function]':
			return console.error('`message` should either be an `Object`, `Array` or a `String`.');
