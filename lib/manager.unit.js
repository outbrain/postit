...
describe('Manager', function () {
	...
	describe('get', function () {
		it('should get an explicit instance', function () {
			...
			expect(Manager.get('foo')).to.contain.all.keys(['instance']);
			expect(Manager.get('bar')).to.contain.all.keys(['instance']);
		});
	...
	describe('on', function () {
		it('should register event listeners', function () {
			...
			expect(Manager.get('foo').instance.listeners['foo.bar']).to.have.length(2);
			expect(Manager.get('foo').instance.listeners['foo.baz']).to.have.length(1);

			expect(Manager.get('bar').instance.listeners['bar.bar']).to.have.length(1);
			expect(Manager.get('bar').instance.listeners['bar.baz']).to.have.length(2);
			expect(Manager.get('foo').instance.listeners['foo.boo']).to.not.exist;
		});
		...
		it('should not register event listeners with an unknown `id`', function () {
			...
			expect(Manager.get('foo').instance.listeners).to.be.empty;
			expect(Manager.get('bar').instance.listeners).to.be.empty;
		});
	});
	...
	describe('off', function () {
		it('should unregister all event listeners', function () {
			...
			expect(Manager.get('foo').instance.listeners['foo.bar']).to.have.length(0);
			expect(Manager.get('bar').instance.listeners['bar.bar']).to.have.length(0);
		});
		...
		it('should unregister explicit event listeners', function () {
			...
			expect(Manager.get('foo').instance.listeners['foo.bar']).to.have.length(2);
			expect(Manager.get('foo').instance.listeners['foo.bar']).to.not.include(listener1);

			expect(Manager.get('bar').instance.listeners['bar.bar']).to.have.length(2);
			expect(Manager.get('bar').instance.listeners['bar.bar']).to.not.include(listener2);
		});
		...
		it('should not unregister event listeners with an unknown `id`', function () {
			...
			expect(Manager.get('foo').instance.listeners['foo.bar']).to.have.length(1);
			expect(Manager.get('bar').instance.listeners['bar.bar']).to.have.length(1);
		});
	});
